...
q1 = pd.pivot_table(data=dataset, columns='waterfront',
...
dataset[['price', 'bedrooms', 'bathrooms', 'sqft_living',
         'sqft_lot', 'floors', 'yr_built']].corr().abs().iloc[1:, 0]
...
q3 = dataset.drop(columns=['id', 'date', 'zipcode'])
...
dataset2 = pd.read_csv("Dataset_07.csv")
dataset2.dropna()
...
q1 = dataset2[['GRE', 'TOEFL', 'CGPA', 'Chance_of_Admit']].corr()
q1.iloc[:-1, -1].max()
...
q3 = dataset2.drop(columns=['Serial_No'])
...
q1_2.columns
q1_3 = pd.pivot_table(data=q1_2, index='model', values=['age_in_days', 'km'])
...
q2['km_per_day'] = q1['km']/q2['age_in_days']

big=q2[q2.model=='lounge']['km_per_day']
small=q2[q2.model=='sprot']['km_per_day']
